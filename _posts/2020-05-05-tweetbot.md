...
* We still need to handle one error for duplicate status, it shouldn’t post the same tweets everytime from the start. To do that, we’ll add a try ... except block to our code, and also have the console print out the reason for the error.
